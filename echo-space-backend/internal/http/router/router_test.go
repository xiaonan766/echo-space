...
func TestWebUcenterActualTimeStatisticsRouteRequiresLogin(t *testing.T) {
	cfg := config.Config{}
	cfg.Server.Mode = "test"
	engine := New(Dependencies{Config: cfg})

	request := httptest.NewRequest(http.MethodPost, "/web/ucenter/getActualTimeStatisticsInfo", nil)
	recorder := httptest.NewRecorder()
	engine.ServeHTTP(recorder, request)
...
func TestWebUcenterWeekStatisticsRouteRequiresLogin(t *testing.T) {
	cfg := config.Config{}
	cfg.Server.Mode = "test"
	engine := New(Dependencies{Config: cfg})

	request := httptest.NewRequest(http.MethodPost, "/web/ucenter/getWeekStatisticsInfo", strings.NewReader("dataType=0"))
	request.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	recorder := httptest.NewRecorder()
	engine.ServeHTTP(recorder, request)
...
func TestReportVideoPlayOnlineRouteDoesNotRequireLogin(t *testing.T) {
	cfg := config.Config{}
	cfg.Server.Mode = "test"
	engine := New(Dependencies{Config: cfg})
...
func TestWebUhomeUpdateUserInfoRouteRequiresLogin(t *testing.T) {
	cfg := config.Config{}
	cfg.Server.Mode = "test"
	engine := New(Dependencies{Config: cfg})

	request := httptest.NewRequest(http.MethodPost, "/web/uhome/updateUserInfo", strings.NewReader("userId=1000000001"))
	request.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	recorder := httptest.NewRecorder()
	engine.ServeHTTP(recorder, request)
